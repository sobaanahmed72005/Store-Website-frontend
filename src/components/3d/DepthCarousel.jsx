...
const DepthCarousel = ({
  ...
  useEffect(() => {
    ...
    const ro = new ResizeObserver((entries) => {
      ...
      const cfg = cfgRef.current
      const neededW = cfg.cardWidth + Math.abs(cfg.spread) * 1.8 + 40
      const neededH = cfg.cardHeight + 40
      const scaleX = w / neededW
      const scaleY = h ? h / neededH : 1
      scaleRef.current = clamp(Math.min(scaleX, scaleY), 0.35, 1.25)
      layout(posRef.current)
    })
    ro.observe(root)
