...
export default function MetallicPaint({
  ...
  const uploadTexture = useCallback(imgData => {
    ...
    // Fit canvas aspect ratio to screen ratio smoothly
    const canvasRatio = (canvasRef.current?.width || 1) / (canvasRef.current?.height || 1);
  ...
  useEffect(() => {
    ...
    const height = (canvas.clientHeight || window.innerHeight) * dpr;
