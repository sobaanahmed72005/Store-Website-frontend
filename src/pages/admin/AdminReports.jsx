...
export default function AdminReports() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      api.get(ENDPOINTS.ADMIN.REPORTS.BOTTOM_PRODUCTS('?limit=10'), { auth: true }).then(setBottomProducts),
      api.get(ENDPOINTS.ADMIN.REPORTS.SALES_BY_CITY('?limit=10'), { auth: true }).then(setSalesByCity),
  ...
  useEffect(() => {
    api.get(ENDPOINTS.ADMIN.REPORTS.REVENUE_TREND(`?period=${trendPeriod}`), { auth: true })
      .then(setTrend)
      .catch((err) => setError(err.message))
  }, [trendPeriod])

  useEffect(() => {
    api.get(ENDPOINTS.ADMIN.REPORTS.TOP_PRODUCTS(`?limit=10&by=${topSortBy}`), { auth: true })
      .then(setTopProducts)
      .catch((err) => setError(err.message))
  }, [topSortBy])
